...
namespace BankApplication
{
    class Program
    {
        ...
        {
            Bank<Account> bank = new Bank<Account>("ДамирБанк");
            bool alive = true;
        ...
        private static void OpenAccount(Bank<Account> bank)
        {
            ...
            bank.Open(accountType,
                ...
                (o, e) => Console.WriteLine(e.Message), // обработчик начислений процентов в виде лямбды-выражения
                CloseAccountHandler, // обработчик закрытия счета
                OpenAccountHandler); // обработчик открытия счет
        }
        ...
        //обработчик событий открытия счета
        private static void OpenAccountHandler(object sender, AccountEvenArgs e)
        {
            Console.WriteLine(e.Message);
        }
        //обработчик добавления денег на счет
        private static void AddSumHandler(object sender, AccountEvenArgs e)
        {
            Console.WriteLine(e.Message);
        }
        // обработчик вывода средств
        private static void WithdrawSumHandler(object sender, AccountEvenArgs e)
        {
            Console.WriteLine(e.Message);
            if (e.Sum > 0)
            {
        ...
        // обработчик вывода средств
        private static void CloseAccountHandler(object sender, AccountEvenArgs e)
        {
            Console.WriteLine(e.Message);
        }
    }
